<template>
  <div id="fake-installer">
    ...
    <label for="version">Choisir une version de Minecraf</label>
    ...
    <input
      type="text"
      id="location"
      placeholder="Comme C:/MinecraftStuff"
      v-model="location"
      :disabled="installing"
    />

    <button @click="startInstall" :disabled="!canInstall || installing">
      {{ installing ? "Installation... Patientez..." : "Installé" }}
    </button>
...
const statusMessage = ref('Choisissez une version et le dossier d\'installation.');
...
function startInstall() {
  ...
  progress.value = 0;
  statusMessage.value = 'Préparation de l\'Instalathion...';

  const interval = setInterval(() => {
    if (progress.value >= 100) {
      clearInterval(interval);
      statusMessage.value = `Minercaft ${version.value} installé sur ${location.value}. Bravo ! 🎉`;
