<template>
  <div id="installer">
    <h1>Installeur de Minecraft</h1>

    <label for="version">Choisissez une version de Minecraft</label>
    <select id="version" v-model="version" :disabled="installing">
      <option disabled value="">-- Choix de version --</option>
...
function startInstall() {
  ...
  progress.value = 0;
  statusMessage.value = 'Preparation de l\'installation...';

  const interval = setInterval(() => {
    if (progress.value >= 100) {
      clearInterval(interval);
      statusMessage.value = `Minecraft ${version.value} installé avec succès dans ${location.value} ! 🎉`;
      ...
      statusMessage.value = `Installation de Minecraft ${version.value}... ${progress.value}%`;
    }
  }, 500);
}
</script>
