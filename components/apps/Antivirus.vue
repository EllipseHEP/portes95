<template>
  <div class="dse-window min-w-2xl max-w-2xl">
    <header class="dse-header">
      <div class="dse-title">Portes Antivirus</div>
      <div class="dse-buttons">
        <button @click="startScan" :disabled="scanning || disinfecting">{{ scanning ? 'Scannage...' : 'Scanner' }}</button>
        <button
        ...
        >
          {{ disinfecting ? 'Désinfection...' : 'Désinfecter' }}
        </button>
      </div>
    </header>
    ...
    <main class="dse-body">
      ...
      <div style="text-align:center; margin-bottom:1.2rem; color:#3a6ea5; font-weight:600;">
        <template v-if="disinfecting">
          Désinfection... {{ disinfectProgress }}%
        </template>
        <template v-else>
          Scannage de {{ scannedFiles }} fichiers parmi {{ totalFiles }} fichiers ({{ scannedFiles }}/{{ totalFiles }})
        </template>
      </div>

      <div v-if="!scanning && !disinfecting && progress === 100 && warnings.length" class="dse-results">
        <div class="dse-warnings-summary">
          <h3>Rapport du scan</h3>
          <ul>
...
const statusClass = computed(() => {
  if (disinfecting.value) return 'Scan en cours'
  if (scanning.value) return 'sScan en cours'
  if (missionInfo.infectionLevel < 25) return 'sûr'
  if (missionInfo.infectionLevel < 50) return 'avertissement'
  return 'infecté'
})

const statusText = computed(() => {
  if (disinfecting.value) return 'Désinfection...'
  if (scanning.value) return 'Scannage...'
  if (missionInfo.infectionLevel < 25) return 'Vous êtes en sécurité'
  if (missionInfo.infectionLevel < 50) return 'Il y a des avertissements'
  if (missionInfo.infectionLevel < 75) return 'Vous êtes infecté'
  return 'Infection sévère détectée'
})

const statusDesc = computed(() => {
  if (disinfecting.value) return 'Tentative de désinfecter votre système. S\'il vous plaît, attendez...'
  if (scanning.value) return 'Portes Antivirus inspecte les menaces sur votre ordinateur.'
  if (missionInfo.infectionLevel < 25) return 'Aucune menace détectée. Votre ordinateur est protégé.'
  if (missionInfo.infectionLevel < 50) return 'Certains problèmes ont été trouvés. Veuillez consulter les avertissements ci-dessous.'
  if (missionInfo.infectionLevel < 75) return 'Menaces détectées ! Une action immédiate est recommandée.'
  return 'Menaces critiques détectées ! Système en danger.'
})
...
function generateWarnings(level) {
  ...
  const warningMessages = [
    'Programme de démarrage suspect détecté',
    'Définitions antivirus obsolètes',
    'Programme potentiellement indésirable trouvé',
    'Activité de réseau inhabituelle',
    'Fichiers temporaires volumineux trouvés',
  ]

  const errorMessages = [
    'Infection par Malware détectée',
    'Fichier système critique corrompu',
    'Tentative d\'accès non autorisé',
    'Signature de ransomware trouvée',
    'Rootkit détecté',
  ]
  ...
  for (let i = 0; i < count; i++) {
    if (level < 50) {
      ...
      issues.push({
        severity: i % 2 === 0 ? 'avertissement' : 'erreur',
